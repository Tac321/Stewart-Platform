...
function [inv_return,LegsSCD]=stew_inverse(xsi,ysi,xmi,ymi,roll,pitch,yaw,px,py,pz,neutralZ)
clc
global grumx grumy grumz grumx2 grumy2 grumz2
rollOrig=roll;
pitchOrig=pitch;
yawOrig=yaw;
pxOrig=px;
...
%%     SCD Print stuff
    for i=1:length(roll)
        ...
        TXrad=roll(i)*pi/180;                  %convert the roll angle to radians
        TYrad=pitch(i)*pi/180;                 %convert the pitch angle to radians
        TZrad=yaw(i)*pi/180;                   %convert the yaw angle to radians

            %T is the transformation (translation + rotation) matrix from Frame B to Frame N.
        T =[cos(TYrad)*cos(TZrad),                                 -cos(TYrad)*sin(TZrad),                                  sin(TYrad),           px(i);
            sin(TXrad)*sin(TYrad)*cos(TZrad)+cos(TXrad)*sin(TZrad),-sin(TXrad)*sin(TYrad)*sin(TZrad)+cos(TXrad)*cos(TZrad),-sin(TXrad)*cos(TYrad),py(i);
           -cos(TXrad)*sin(TYrad)*cos(TZrad)+sin(TXrad)*sin(TZrad), cos(TXrad)*sin(TYrad)*sin(TZrad)+sin(TXrad)*cos(TZrad), cos(TXrad)*cos(TYrad),pz(i)-neutralZ;
            0,                                                      0,                                                      0,                    1 ];

            %Ta is the transformation matrix that rotates and translates the
            %platform to the correct orientation for animation.
        Ta =[cos(TYrad)*cos(TZrad),                                 -cos(TYrad)*sin(TZrad),                                  sin(TYrad),           px(i);
             sin(TXrad)*sin(TYrad)*cos(TZrad)+cos(TXrad)*sin(TZrad),-sin(TXrad)*sin(TYrad)*sin(TZrad)+cos(TXrad)*cos(TZrad),-sin(TXrad)*cos(TYrad),py(i);
            -cos(TXrad)*sin(TYrad)*cos(TZrad)+sin(TXrad)*sin(TZrad), cos(TXrad)*sin(TYrad)*sin(TZrad)+sin(TXrad)*cos(TZrad), cos(TXrad)*cos(TYrad),neutralZ-pz(i);
             0,                                                      0,                                                      0,                    1 ];
        ...
        Legs(i,:)=[L1,L2,L3,L4,L5,L6];   %leg value return
    ...
    T =[cos(-TYrad)*cos(TZrad),                                 -cos(-TYrad)*sin(TZrad),                                  sin(-TYrad),           px;
       ...
       -cos(TXrad)*sin(-TYrad)*cos(TZrad)+sin(TXrad)*sin(TZrad), cos(TXrad)*sin(-TYrad)*sin(TZrad)+sin(TXrad)*cos(TZrad), cos(TXrad)*cos(-TYrad),pz-neutralZ;
        0,                                                      0,                                                      0,                    1 ];
    ...
    Ta =[cos(TYrad)*cos(TZrad),                                 -cos(TYrad)*sin(TZrad),                                  sin(TYrad),           px;
         sin(TXrad)*sin(TYrad)*cos(TZrad)+cos(TXrad)*sin(TZrad),-sin(TXrad)*sin(TYrad)*sin(TZrad)+cos(TXrad)*cos(TZrad),-sin(TXrad)*cos(TYrad),py;
        -cos(TXrad)*sin(TYrad)*cos(TZrad)+sin(TXrad)*sin(TZrad), cos(TXrad)*sin(TYrad)*sin(TZrad)+sin(TXrad)*cos(TZrad), cos(TXrad)*cos(TYrad),neutralZ-pz;
         0,                                                      0,                                                      0,                    1 ];
